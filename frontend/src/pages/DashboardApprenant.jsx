...
function DashboardApprenant({ user }) {
  ...
  useEffect(() => {
    ...
    const loadFormations = async () => {
      try {
        ...
        if (active) setFormations(data)
      } catch (loadError) {
        if (active) setError(loadError.message || 'Impossible de charger vos formations.')
      } finally {
  ...
  const confirmerDesinscription = async () => {
    if (!enrollmentToRemove?.formation?.id) return
    ...
    try {
      ...
      setActionError(err.message || 'Impossible de vous désinscrire.')
    } finally {
  ...
  if (loading) {
    ...
    content = (
      <div className="text-center py-5">
        ...
        <Link to="/formations" className="sh-btn sh-btn--primary">Voir les formations</Link>
      </div>
    )
  } else {
    content = (
      <div className="row g-4">
        {formations.map((item) => {
          ...
          const progressBackground = progress >= 80 ? 'var(--green-text)' : progress >= 40 ? '#f59e0b' : 'var(--brand-main)'
              ...
              <div className="sh-formation-card">
                ...
                <div className="mb-2">
                  ...
                  <div style={{ height: 6, background: 'var(--brand-border)', borderRadius: 999 }}>
                    <div style={{ height: '100%', width: `${progress}%`, background: progressBackground, borderRadius: 999, transition: 'width 0.3s ease' }} />
                  </div>
                </div>
                <div className="d-flex gap-2 mt-2">
                  <Link to={`/apprendre/${formation.id}`} className="sh-btn sh-btn--primary flex-fill" style={{ fontSize: 12, padding: '7px 10px' }}>
                    Suivre
                  </Link>
                  <button
  ...
  return (
    <div>
      <section className="sh-section--dark py-5">
        <div className="container">
          <div className="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
              <h1 className="sh-section-title--light mb-1">Bonjour, {user.name || user.email} 👋</h1>
              <p className="sh-section-sub--light">Suivez vos formations et gérez vos inscriptions</p>
            </div>
            <Link to="/formations" className="sh-btn sh-btn--white">Découvrir des formations</Link>
          </div>
        </div>
      </section>
      ...
      <Modal show={showConfirm} onHide={() => setShowConfirm(false)} centered>
        ...
        <Modal.Body>
          <p>Êtes-vous sûr de vouloir vous désinscrire ? Votre progression sera perdue.</p>
          {actionError && <div className="alert alert-danger mt-3 mb-0 py-2">{actionError}</div>}
        </Modal.Body>
        <Modal.Footer>
          <Button variant="outline-secondary" onClick={() => setShowConfirm(false)}>Annuler</Button>
